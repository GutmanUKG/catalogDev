...
class CModuleOptions
{
    ...
    private function SaveOptions()
    {
        ...
        if (!empty($_REQUEST['VERSION_TEMPLATE']) && 
            $_REQUEST['VERSION_TEMPLATE'] != COption::GetOptionString($this->module_id, 'VERSION_TEMPLATE', 'v1', $this->siteId)) {
            self::rewritePublicFiles($_REQUEST['VERSION_TEMPLATE']);
        }
        ...
        foreach ($this->arOptions as $opt => $arOptParams) {
            if ($arOptParams['TYPE'] != 'CUSTOM') {
                ...
                if ($arOptParams['TYPE'] == 'CHECKBOX' && $val != 'Y') {
                    $val = 'N';
                } elseif ($arOptParams['TYPE'] == 'IMG' && is_array($val)) {
                    $val = array_merge($val, ['del' => $opt . '_del', "MODULE_ID" => "sotbit.b2bcabinet"]);
                    $val['tmp_name'] = CTempFile::GetAbsoluteRoot() . $val['tmp_name'];
                    $val = CFile::SaveFile($val, 'sotbit_b2bcabinet');
                } elseif (is_array($val)) {
                    $val = serialize($val);
                }
    ...
    private function SaveGroupRight()
    {
        ...
        foreach ($GROUP as $k => $v) {
            if ($k == 0) {
                COption::SetOptionString($this->module_id, 'GROUP_DEFAULT_RIGHT', $RIGHT[0],
    ...
    private function GetCurOptionValues()
    {
        foreach ($this->arOptions as $opt => $arOptParams) {
            if ($arOptParams['TYPE'] != 'CUSTOM') {
                $this->arCurOptionValues[$opt] = COption::GetOptionString($this->module_id, $opt,
                    $arOptParams['DEFAULT'], $this->siteId);

                if (in_array($arOptParams['TYPE'], ['MSELECT'])) {
                    $this->arCurOptionValues[$opt] = unserialize($this->arCurOptionValues[$opt], ['allowed_classes' => false]);
                }
            }
        }
    }
    ...
    public function ShowHTML()
    {
        ...
        foreach ($this->arGroups as $group_id => $group_params) {
            $arP[$group_params['TAB']][$group_id]['SETTINGS'] = $group_params;

        }

        if (is_array($this->arOptions)) {
            foreach ($this->arOptions as $option => $arOptParams) {
                if (!$arOptParams || $arOptParams['FROM_FILE'] === 'Y') {
                    continue;
                }
                ...
                switch ($arOptParams['TYPE']) {
                    ...
                    case 'TEXT' :
                        ...
                        $input = '<textarea cols="' . $arOptParams['COLS'] . '" rows="' . $arOptParams['ROWS'] . '" name="' . $opt . '">' . htmlspecialchars($val,
                                null, LANG_CHARSET) . '</textarea>';
                    ...
                    case 'SELECT' :
                        $input = SelectBoxFromArray($opt, $arOptParams['VALUES'], $val, '',
                            ($arOptParams['OTHER'] == 'Y') ? 'onchange="SelectOther(this.value,\'' . $option . '\')"' : '',
                    ...
                    case 'MSELECT' :
                        ...
                        $input = SelectBoxMFromArray($opt . '[]', $arOptParams['VALUES'], $val, '', false,
                            $arOptParams['SIZE'], $selHTML);
                    ...
                    case 'FILE' :
                        ...
                        break;
                    case 'IMG' :
                        $input = \Bitrix\Main\UI\FileInput::createInstance(array(
                            "name" => $opt,
                    ...
                    default :
                        ...
                        if ($arOptParams['MULTI'] == 'N') {
                            ...
                            if (isset($val) && !empty($val)) {
                                $massive = unserialize($val, ['allowed_classes' => false]);
                            } else {
                                $massive = array();
                            }
...
document.getElementById('" . htmlspecialchars($option) . "').appendChild(div);
            ...
            foreach ($arP as $tab => $groups) {
                ...
                foreach ($groups as $group_id => $group) {
                    if ($group['SETTINGS']['FILE']) {
                        require_once $_SERVER['DOCUMENT_ROOT'] . '/bitrix/modules/' . $this->module_id . $group['SETTINGS']['FILE'];
                        continue;
                    }
    ...
    private static function getPersonType($arId = [])
    {
        ...
        if ($arId) {
            $query->whereIn('ID', $arId);
        }

        return array_column($query->fetchAll(), 'NAME', 'ID');
    }

    public static function rewritePublicFiles($versionB2bTemplate = 'v1') 
    {   
        $WIZARD_RELATIVE_PATH = '/bitrix/modules/sotbit.b2bcabinet/install/wizards/sotbit/b2bcabinet';
        $filePath = $_SERVER["DOCUMENT_ROOT"].$WIZARD_RELATIVE_PATH.'/site/public/ru/';
        $methodInstall = Config::getMethodInstall(htmlspecialchars($_REQUEST['site']));
        $filePathVersionB2b = $versionB2bTemplate === 'v2' ? '_v2/' : '/';
        $templateName = $versionB2bTemplate === 'v2' ? 'b2bcabinet_v2.0' : 'b2bcabinet';
        ...
        if($methodInstall == 'AS_SITE') {
            CopyDirFiles($filePath . "site" . $filePathVersionB2b, $_SERVER["DOCUMENT_ROOT"].$SITE_DIR, true, true);
            RewriteFile($_SERVER["DOCUMENT_ROOT"].$SITE_DIR.'index.php', \Bitrix\Main\IO\File::getFileContents($filePath . "site" . $filePathVersionB2b . '_index.php'));
        }
        else if ($methodInstall == 'AS_TEMPLATE') {
            if(!is_dir($_SERVER["DOCUMENT_ROOT"].$SITE_DIR."b2bcabinet/")) {
                mkdir($_SERVER["DOCUMENT_ROOT"].$SITE_DIR."b2bcabinet/", 0775, true);
            }
        
            CopyDirFiles($filePath . "b2bcabinet" . $filePathVersionB2b, $_SERVER["DOCUMENT_ROOT"].$SITE_DIR."b2bcabinet/", true, true);
        }

        CopyDirFiles($filePath . "common/gadgets/" . $versionB2bTemplate, $_SERVER["DOCUMENT_ROOT"].$SITE_DIR.'local/', true, true);

        self::setTemplate($templateName);
    }

    private static function setTemplate($newTemplate) {
        $rsTemplates = CSite::GetTemplateList($_REQUEST['site']);
        $result = [];
        while($arTemplate = $rsTemplates->Fetch()) {
            if (preg_match('#b2bcabinet#', $arTemplate["TEMPLATE"])) {
                $arTemplate["TEMPLATE"] = $newTemplate;
            }
            $result[] = $arTemplate;
        }
