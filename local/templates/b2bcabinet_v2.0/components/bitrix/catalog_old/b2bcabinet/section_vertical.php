...
include $_SERVER["DOCUMENT_ROOT"] . "/" . $this->GetFolder() . "/params_modifier.php";
...
<?if ($isFilter):?>
    ...
    <aside class="offcanvas-xxxl offcanvas-size-lg offcanvas-end
        ...
        <div class="sidebar-content bx_filter<?= (isset($arParams['FILTER_HIDE_ON_MOBILE']) && $arParams['FILTER_HIDE_ON_MOBILE'] === 'Y' ? ' hidden-xs' : '') ?>">
            <?
            // При поиске умный фильтр пишет в отдельную переменную
            $smartFilterName = !empty($_REQUEST["q"]) ? "smartFilterTemp" : $arParams["FILTER_NAME"];
            $APPLICATION->IncludeComponent(
                ...
                array(
                    ...
                    "PREFILTER_NAME" => !empty($_REQUEST["q"]) ? "" : $arParams["FILTER_NAME"],
                    "FILTER_NAME" => $smartFilterName,
...
<div id="card__catalog__section-wrapper" class="catalog__section-wrapper <?if($session['FULLSCREEN'] == "Y"):?>card-fullscreen<?endif;?>">
    ...
    <section class="catalog__section <?= $arParams["CATALOG_NOT_AVAILABLE"] === "Y" ? "catalog__section-not_available" : '' ?>">
        ...
        // При поиске используем только фильтр поиска (умный фильтр отображается, но не применяется)
        global $searchFilter;
        if (!empty($_REQUEST["q"]) && !empty($searchFilter["ID"])) {
            $filterName = $arParams["FILTER_NAME"];
            $GLOBALS[$filterName] = $searchFilter;
        }
        ...
        $intSectionID = $APPLICATION->IncludeComponent(
            ...
            array(
                ...
                "CACHE_TYPE" => !empty($_REQUEST["q"]) ? "N" : $arParams["CACHE_TYPE"],
                "CACHE_TIME" => $arParams["CACHE_TIME"],
                "CACHE_FILTER" => !empty($_REQUEST["q"]) ? "Y" : $arParams["CACHE_FILTER"],
                "CACHE_GROUPS" => $arParams["CACHE_GROUPS"],
                "CATALOG_NOT_AVAILABLE" => $arParams["CATALOG_NOT_AVAILABLE"],
                ...
                'SECTION_ID' => !empty($_REQUEST["q"]) ? false : $arResult['VARIABLES']['SECTION_ID'],
                'SECTION_CODE' => !empty($_REQUEST["q"]) ? '' : $arResult['VARIABLES']['SECTION_CODE'],
