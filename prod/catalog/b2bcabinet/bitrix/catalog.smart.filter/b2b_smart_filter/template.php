...
<form name="<? echo $arResult["FILTER_NAME"] . "_form" ?>" action="<? echo $arResult["FORM_ACTION"] ?>"
    ...
    <div class="index_blank-filter">
        <div class="bx_filter_section bx_filter">
            ...
            <div class="find_property_value_wrapper form-control-feedback form-control-feedback-start" style="display: block;
            width: calc(100% - 2rem);
            margin: 0 auto;">
                <input type="text" class="form-control find_property_value"
                       placeholder="Поиск" id="search_filter">
                                ...
                                <div class="bx_filter_parameters_box_container row">
                                    <div class="bx_filter_parameters_box_container_block">
                                        <div class="bx_filter_input_container">
                                            <label for="<?= $arItem["VALUES"]["MIN"]["CONTROL_ID"] ?>"><?= Loc::getMessage('CT_BCSF_FILTER_FROM') ?></label>
                                            <input
                                    ...
                                    <div class="bx_filter_parameters_box_container_block">
                                        <div class="bx_filter_input_container">
                                            <label for="<?= $arItem["VALUES"]["MAX"]["CONTROL_ID"]?>"><?= Loc::getMessage('CT_BCSF_FILTER_TO') ?></label>
                                            <input
...
<!--                                        <input type="text" class="form-control find_property_value"-->
<!--                                               placeholder="--><?php //= GetMessage('CT_BCSF_FILTER_SELECT') ?><!--">-->
<!--                                        <button class="form-control-feedback-icon find_property_value__button find_property_value__button-search">-->
<!--                                            <i class="ph-magnifying-glass find_property_value__icon-search"></i>-->
<!--                                        </button>-->
<!--                                        <button class="form-control-feedback-icon find_property_value__button find_property_value__button-close">-->
                                    ...
                                    <div class="blank_ul_wrapper type-checkbox <?= ($totalValues > $maxDisplay ? "perfectscroll" : "") ?>">
                                        ...
                                        foreach ($arItem["VALUES"] as $val => $ar):
                                            ...
                                            <?if(empty($ar["DISABLED"])):?>
                                                ...
                                                <?if($ch == 3 && count($arItem["VALUES"]) > $ch):?>
                                                    <button class="bx_filter_parameters_box_collapse_btn 22 collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseBox-<?=$arItem["ID"]?>" aria-expanded="false" aria-controls="collapseBox-<?=$arItem["ID"]?>">
                                                        Показать ещё
                                                    </button>
                                                    <div class="collapse bx_filter_parameters_box_collapse" id="collapseBox-<?=$arItem["ID"]?>">
                                                <?endif;?>

                                                <?if($ch > 3 && $ch == count($arItem["VALUES"])):?>
                                                    </div>
                                                <?endif;?>
                                            <?endif;?>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.bx_filter_parameters_box').forEach(box => {
        const allItems = box.querySelectorAll('.form-check'); // или другой селектор чекбоксов

        const collapseBtn = box.querySelector('.bx_filter_parameters_box_collapse_btn');
        const collapseContainer = box.querySelector('.bx_filter_parameters_box_collapse');

        if (!collapseBtn || !collapseContainer) return;

        if (allItems.length <= collapseLimit) {
            // Показываем всё, скрываем collapse и кнопку
            collapseBtn.style.display = 'none';
            collapseContainer.style.display = 'none';
        } else {
            // Скрываем все, кроме первых 3
            allItems.forEach((item, index) => {
                if (index >= collapseLimit) {
                    collapseContainer.appendChild(item); // переносим в collapse
                }
            });
            ...
            collapseBtn.addEventListener('click', () => {
                if (collapseBtn.classList.contains('collapsed')) {
                    collapseBtn.textContent = 'Показать ещё';
                } else {
                    collapseBtn.textContent = 'Скрыть';
                }
            });
