    ...
    )
    {
        ...
        if($ids)
        {
            ...
            foreach($ids as $id)
            {
                ...
                if(empty($Image)) {
                    $rsCatalogSku = \CCatalogSku::GetProductInfo($id);
                    if (is_array($rsCatalogSku)) {
                        $rsElement = \CIBlockElement::GetList(array(), array(
                            "ID" => $rsCatalogSku['ID']
                        ), false, false, array(
                            "ID",
                        ...
                        while ($elem = $rsElement->Fetch())
                        {
                            if($elem['PREVIEW_PICTURE'])
                            {
                                $Image = $elem['PREVIEW_PICTURE'];
                            }
                            elseif($elem['DETAIL_PICTURE'])
                            {
                                $Image = $elem['DETAIL_PICTURE'];
                            }
                        }
                    }
                }
                ...
                else
                {
                    $noPhoto = \CFile::MakeFileArray($imgProp['noPhoto']);
                    $noPhoto['MODULE_ID'] = 'sotbit.b2bcabinet';
                    $imgNoPhoto = \CFile::SaveFile($noPhoto, 'sotbit.b2bcabinet');
                    $noPhoto['ID'] = $imgNoPhoto;
