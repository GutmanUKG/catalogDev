...
if($idUr > 0)
{
    ...
    Option::set("sotbit.auth", "GROUP_FIELDS_FOR_CHECK_".$idUr, serialize(array('INN')), WIZARD_SITE_ID);
...
if($idIp > 0)
{
    ...
    Option::set("sotbit.auth", "GROUP_FIELDS_FOR_CHECK_".$idIp, serialize(array('INN')), WIZARD_SITE_ID);
...
$authMailEvents = array_column(\Bitrix\Main\Mail\Internal\EventMessageTable::query()
    ->addSelect('ID')
    ->whereLike('EVENT_NAME', 'SOTBIT_AUTH_%')
    ->fetchAll() ?: [], 'ID');

if ($authMailEvents) {
    array_walk($authMailEvents, fn($id) => \Bitrix\Main\Mail\Internal\EventMessageTable::update($id, ['SITE_TEMPLATE_ID' => 'b2bcabinet_mail']));
}