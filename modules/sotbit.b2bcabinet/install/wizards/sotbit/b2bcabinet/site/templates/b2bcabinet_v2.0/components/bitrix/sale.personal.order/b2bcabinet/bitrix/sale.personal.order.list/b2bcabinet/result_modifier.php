...
$buyers = [];
$pt = unserialize(Option::get("sotbit.b2bcabinet","BUYER_PERSONAL_TYPE","a:0:{}"), ['allowed_classes' => false]);
if(!is_array($pt))
{
	$pt = [];
}

$innProps = unserialize(Option::get('sotbit.b2bcabinet', 'PROFILE_ORG_INN'), ['allowed_classes' => false]);
...
$orgProps = unserialize(Option::get('sotbit.b2bcabinet', 'PROFILE_ORG_NAME'), ['allowed_classes' => false]);
...
if( $filterData['BUYER'] )
{
    ...
    {
        $innV = [];
        $innProps = unserialize( Bitrix\Main\Config\Option::get( 'sotbit.b2bcabinet', 'PROFILE_ORG_INN' ) , ['allowed_classes' => false]);
        if( !is_array( $innProps ) )
        {
            $innProps = [];
        }
