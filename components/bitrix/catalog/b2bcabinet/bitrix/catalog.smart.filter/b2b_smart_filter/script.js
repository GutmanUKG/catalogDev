...
document.addEventListener('DOMContentLoaded', ()=>{
  let filter = document.forms.arrFilter_form;
  let filterItem = filter.querySelectorAll('.bx_filter_parameters_box_checkbox');
  const popup_filter = document.createElement('div')
  const setFilter = filter.querySelector('#set_filter');
  popup_filter.classList.add('popup_filter', 'bordered')
  popup_filter.style.cssText = 'position:absolute; z-index: 9999;'

  let countItem = 0;
  let countResult = document.createElement('span')

  countResult.classList.add('count_result')
  document.body.appendChild(popup_filter)
  let submit = document.createElement('input');
  submit.type = 'button'
  submit.value = 'Показать'
  popup_filter.style.cssText += 'display:none';
  submit.classList.add('btn', 'btn-primary')

  popup_filter.appendChild(submit)
  //popup_filter.appendChild(countResult)
  filterItem.forEach(i=>{
    i.addEventListener('click', ()=>{
      let x = i.getBoundingClientRect().x,
          y = i.getBoundingClientRect().y;
      countItem = filter.querySelector('#modef_num').textContent;
      popup_filter.style.cssText += `top: ${y}px; left: ${x}px; transform: translateX(calc(-100% - 20px)); display:block`
      countResult.textContent = `Найденно: ${countItem}` ;
    })
  })
