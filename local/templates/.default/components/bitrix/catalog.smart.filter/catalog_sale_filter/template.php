...
<div class="bx_filter <?=$templateData["TEMPLATE_CLASS"]?> bx_horizontal">
	<div class="bx_filter_section">
        <div class="card-header d-flex align-items-center">
            <span class="text-uppercase fw-medium fs-md">Фильтр</span>
        </div>
					...
					<div class="bx_filter_block">
						...
						switch ($arItem["DISPLAY_TYPE"])
						{
							...
							default://CHECKBOXES
                                ...
                                <?if($arItem["NAME"] !== 'Категория уценки'){
                                    $limit = 4;
                                }?>
                                <?if($count < $limit):?>
								...
								<?else:?>
                                <?if($count <= $limit && $count < count($arItem["VALUES"])):?>
                                <button class="btn_more bx_filter_parameters_box_collapse_btn collapsed">Показать еще</button>
                                <?endif;?>
                                    <label data-role="label_<?=$ar["CONTROL_ID"]?>" class="bx_filter_param_label hide <? echo $ar["DISABLED"] ? 'disabled': '' ?>" for="<? echo $ar["CONTROL_ID"] ?>">
										<span class="bx_filter_input_checkbox">
											<input
                                                    type="checkbox"
                                                    value="<? echo $ar["HTML_VALUE"] ?>"
...
<!--                            --><?//if(!isset($arResult["ELEMENT_COUNT"])) echo 'style="display:none"';?><!-- style="display: inline-block;">-->
