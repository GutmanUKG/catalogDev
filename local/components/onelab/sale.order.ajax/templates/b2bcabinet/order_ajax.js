...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
			...
			else
			{
				...
				BX.ajax({
					...
					onsuccess: BX.delegate(function(result) {
						...
						BX('bx-soa-order-form').querySelector('input[type=hidden][name=profile_change]').value = 'N';
					}, this),
			...
			if (res && res.order)
			{
				...
				else
				{
					if (result.REDIRECT_URL && result.REDIRECT_URL.length)
					{
						...
						let payments = document.querySelector('#bx-soa-paysystem');
						let bck = document.querySelector('#ID_PAY_SYSTEM_ID_12');
						let bckWrap = document.querySelector('#bck');
						try {
							 if(bckWrap){

								let client_email = document.querySelector('input[name="ORDER_PROP_7"]').value || '',
									client_bin = document.querySelector('input[name="ORDER_PROP_4"]').value || '',
									client_phone = document.querySelector('input[name="ORDER_PROP_8"]').value || '',
									amount = '',
									address = '',
									partnerBin = '880801351308';
								 console.log(client_email)
								const url = new URL('https://business-test.bcc.kz/factoring-client-ui/');
								url.search = new URLSearchParams({
									...
									activeTerm: '6',
									address: 'Алматы',
									partnerBin: '180440011718',
								}).toString();

								let frame = document.getElementById('iframe-factoring');
								if (!frame) {
									frame = document.createElement('iframe');
									frame.id = 'iframe-factoring';
									frame.title = 'BCC Factoring';
		...
		editOrder: function()
		{
			...
			this.updateOrderVolumeFromAjax();
			this.updateOrderBoxesFromAjax();
			this.updateRecommendedPickup();
		...
		createDeliveryItem: function(item)
		{
			...
			itemNode = BX.create('DIV', {
				props: {className: 'bx-soa-pp-company col-6 col-sm-3'},
				children: [label, title],
		...
		selectDelivery: function(event)
		{
			...
			// Чтобы не сбрасывало к началу страницы
			// this.sendRequest();
		},
		...
		updateOrderVolumeFromAjax: function() {
			var self = this; // Сохраняем ссылку на объект
			...
			$.ajax({
				...
				success: function(data) {
					...
					if (typeof data.VOLUME !== "undefined") {
						self.result.TOTAL.ORDER_VOLUME = parseFloat(data.VOLUME).toFixed(3) + " м³";
						
						
						// После получения данных обновляем блок с итогами
						self.editTotalBlock();
					} else {
						console.log("VOLUME не найден в ответе AJAX.");
					}
				},
				error: function(xhr, status, error) {
					console.log("Ошибка запроса AJAX:", status, error);
				}
			});
		},

		updateOrderBoxesFromAjax: function() {
			var self = this; // Сохраняем ссылку на объект
			...
			$.ajax({
				...
				dataType: 'json',
				success: function(data) {
					console.log("Данные из AJAX:", data);
		
					if (typeof data.QUANTITY !== "undefined") {
						self.result.TOTAL.ORDER_BOXES = parseInt(data.QUANTITY) + " шт.";
						console.log("Общее количество коробок обновлено:", self.result.TOTAL.ORDER_BOXES);
						
						// После получения данных обновляем блок с итогами
						self.editTotalBlock();
					} else {
						console.log("QUANTITY не найден в ответе AJAX.");
					}
				},
				error: function(xhr, status, error) {
					console.log("Ошибка запроса AJAX:", status, error);
		...
		updateRecommendedPickup: function() {
			var self = this;
			...
			$.ajax({
				...
				success: function(data) {
					...
					if (typeof data.WEIGHT !== "undefined" && typeof data.VOLUME !== "undefined") {
						...
						// Функция расчета процента загрузки
						function calculatePercentage(volume, divisor) {
							let percentage = (volume / divisor);
							return percentage < 10 ? percentage.toFixed(2) + "%" : Math.round(percentage) + "%";
						}
		
						if (weight < 23 && volume < 0.15) {
							tooltipText = "Вес и объем позволяют доставить заказ в руках";
							recommendedHtml = '<div class="truck-container" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-black" data-bs-placement="top" title="' + tooltipText + '">' +
											  '<img src="/local/templates/b2bcabinet_v2.0/assets/images/boxman3.png" ' +
											  'id="ico-boxman" style="width: 60px;">' +
											  '<span class="truck-number">✔</span>' +
											  '</div>';
						} else if (weight >= 23 && volume >= 0.15 && volume < 0.5) {
							tooltipText = "Вес и объем позволяют доставить заказ в багажнике машины";
							recommendedHtml = '<div class="truck-container" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-black" data-bs-placement="top" title="' + tooltipText + '">' +
											  '<img src="/local/templates/b2bcabinet_v2.0/assets/images/car1.png" ' +
											  'id="ico-auto-passenger" style="width: 100px;">' +
											  '<span class="truck-number">✔</span>' +
											  '</div>';
						} else if (volume > 0.5) {
							var smallTruckPercent = calculatePercentage(volume, 0.09);
							var mediumTruckPercent = calculatePercentage(volume, 0.25);
							var bigTruckPercent = calculatePercentage(volume, 0.9);
		
							if (parseFloat(smallTruckPercent) <= 100) {
								tooltipText = `ГАЗель, 1.5 тонны, 9 м³ (${smallTruckPercent} заполнения)`;
								recommendedHtml = '<div class="truck-container" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-black" data-bs-placement="top" title="' + tooltipText + '">' +
												  '<img src="/local/templates/b2bcabinet_v2.0/assets/images/truck-small.png" ' +
												  'class="truck-s" style="width: 100px;">' +
												  '<span class="truck-number">' + smallTruckPercent + '</span>' +
												  '</div>';
							} else if (parseFloat(mediumTruckPercent) <= 100) {
								tooltipText = `Пятитонник, 5 тонн, 25 м³ (${mediumTruckPercent} заполнения)`;
								recommendedHtml = '<div class="truck-container" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-black" data-bs-placement="top" title="' + tooltipText + '">' +
												  '<img src="/local/templates/b2bcabinet_v2.0/assets/images/truck111.png" ' +
												  'class="truck-m" style="width: 100px;">' +
												  '<span class="truck-number">' + mediumTruckPercent + '</span>' +
												  '</div>';
							} else {
								tooltipText = `Еврофура, 20 тонн, 90 м³ (${bigTruckPercent} заполнения)`;
								recommendedHtml = '<div class="truck-container" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-black" data-bs-placement="top" title="' + tooltipText + '">' +
												  '<img src="/local/templates/b2bcabinet_v2.0/assets/images/truck-big.png" ' +
												  'class="truck-b" style="width: 120px;">' +
												  '<span class="truck-number">' + bigTruckPercent + '</span>' +
												  '</div>';
							}
						}
		
						// Сохраняем данные и обновляем блок с итогами
						self.result.TOTAL.RECOMMENDED_PICKUP = recommendedHtml;
						console.log("Рекомендуемый способ забора обновлен:", recommendedHtml);
						self.editTotalBlock();
		
						// После обновления включаем tooltips
						setTimeout(() => {
							var tooltipTriggerList = document.querySelectorAll('[data-bs-toggle="tooltip"]');
							tooltipTriggerList.forEach(el => new bootstrap.Tooltip(el));
						}, 500);
					} else {
						console.log("WEIGHT или VOLUME не найдены в ответе AJAX.");
					}
				},
				error: function(xhr, status, error) {
					console.log("Ошибка запроса AJAX:", status, error);
		...
		editTotalBlock: function()
		{
			...
			if (this.result.TOTAL.ORDER_VOLUME) {
				this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_VOLUME_SUM'), this.result.TOTAL.ORDER_VOLUME));
			}

			if (this.result.TOTAL.ORDER_BOXES) {
				this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_TOTAL_BOXES'), this.result.TOTAL.ORDER_BOXES));
			}
			
			if (this.result.TOTAL.RECOMMENDED_PICKUP) {
				this.totalInfoBlockNode.appendChild(
					this.createTotalUnit(BX.message('SOA_SUM_RECOMMENDED_PICKUP'), this.result.TOTAL.RECOMMENDED_PICKUP)
				);
			}
