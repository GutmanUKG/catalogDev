		...
		<div id="bx-soa-order" class="row bx-<?=$arParams['TEMPLATE_THEME']?>" style="opacity: 0">
			...
			<div id="bx-soa-total" class="col-xxl-3 col-xl-4 col-lg-4 d-none d-lg-flex">
				<div class="bx-soa-sidebar">
					...
					<div class="bx-soa-cart-total-title gradient-modal text-white 23232"><?=Loc::getMessage('CART_TITLE')?></div>
					<div class="bx-soa-cart-total"></div>

                    <?global $USER;
                    if($USER->IsAdmin()):?>
                    <div id="btn_bck" class="btn btn-primary">Оформить рассрочку</div>
                    <?endif;?>
				</div>
			</div>
		</div>
...
?>
    <script>
        document.addEventListener('DOMContentLoaded', ()=>{
            const btnPayment = document.querySelector('#btn_bck');

            btnPayment.addEventListener('click', showBCKPayment)
            function showBCKPayment(e){
                let soa_total = document.querySelector('.bx-soa-cart-total-line-total'),
                    total = soa_total.querySelector('.bx-soa-cart-d').textContent,
                    num = parseFloat(total.replace(/[^\d.,-]/g, '').replace(',', '.'));
                let client_email = document.querySelector('input[name="ORDER_PROP_7"]').value || '',
                    client_bin = document.querySelector('input[name="ORDER_PROP_16"]')
                        ? document.querySelector('input[name="ORDER_PROP_16"]').value
                        : '',

                    client_phone = document.querySelector('input[name="ORDER_PROP_8"]').value || '',//номер +7
                    address = 'Алматы',
                    partnerBin = '021240005281';
                    if(client_bin.length == 0){
                        client_bin = document.querySelector('input[name="ORDER_PROP_4"]')
                            ? document.querySelector('input[name="ORDER_PROP_4"]').value
                            : '';
                    }
                if(client_phone.indexOf('+') == -1){
                    client_phone = '+' + client_phone;
                }
                client_bin = 850823350087;
