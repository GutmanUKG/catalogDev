...
<script>
    ...
    function renderRowItem(product) {
        var rowCount = $('#complaint-positions__grid').DataTable().rows().count();
        var arRow = [<? echo "`". implode("`,`", $arResult["COLUMN_MODEL"]) . "`"?>];

        $('#complaint-positions__grid').DataTable().row.add(arRow).draw();
        App.initSelect2();
    }
</script>
