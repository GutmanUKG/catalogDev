										...
										foreach ($arResult['COLUMNS'] as $id => $header):
											...
											?><td class="<?=$colLayout["cell"]["class"]?>"<?=$colLayout["cell"]["attributes"]?> data-column-id="<?= $header["id"] ?>"><?
												?><div class="main-grid-cell-inner">
													<?
														if($header['id'] == 'PICT') {
															$arSelect = Array("PREVIEW_PICTURE", 'IBLOCK_ID', 'ID', 'PROPERTY_CML2_ARTICLE');
															$arFilter = Array("IBLOCK_ID"=>7,   "PROPERTY_CML2_ARTICLE" => $arRow['data']['ARTICLE']);
															$res = CIBlockElement::GetList(Array("SORT"=>"ASC"), $arFilter, false, ["nPageSize"=>1], $arSelect);
															while($ob = $res->GetNextElement())	{
																$arFields = $ob->GetFields();
															}
															$pictSrc = CFile::GetPath($arFields["PREVIEW_PICTURE"]);
													?>
														<!-- <a class="lightbox-toggle" data-gallery="product" href="<?=$pictSrc?>" data-bs-toggle="lightbox"> -->
															<img class="main-grid-cell-pict" src="<?=$pictSrc?>" alt="<?=$arRow['data']['ARTICLE']?>">
														<!-- </a> -->
													<?
														}
													?>
