...
class OrderTemplatesList extends CBitrixComponent
{
    ...
    public function onPrepareComponentParams($params)
    {
        ...
        $this->personType = unserialize(Config::get('BUYER_PERSONAL_TYPE', SITE_ID), ['allowed_classes' => false]);
    ...
    protected function initialParams()
    {
        foreach ($this->personType as $type) {
            $this->arParams['USER_UF_FIELDS'][$type] = unserialize(Config::get('USER_DOP_FIELDS_' . $type), ['allowed_classes' => false]) ?: [];
            $this->arParams['USER_REGISTER_FIELDS'][$type] = array_unique(array_merge($this->arParams['USER_REGISTER_DEFAULT_FIELDS'],
                unserialize(Config::get('GROUP_FIELDS_' . $type), ['allowed_classes' => false]) ?: [], $this->arParams['USER_UF_FIELDS'][$type]));
            $this->arParams['USER_REQUIRED_FIELDS'][$type] = array_unique(array_merge($this->arParams['USER_REQUIRED_DEFAULT_FIELDS'],
                unserialize(Config::get('GROUP_REQUIRED_FIELDS_' . $type), ['allowed_classes' => false]) ?: []));
            $this->arParams['WHOLESALER_FIELDS'][$type] = unserialize(Config::get('GROUP_ORDER_FIELDS_' . $type), ['allowed_classes' => false]) ?: [];
