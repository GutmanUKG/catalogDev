            ...
            <? foreach ($arResult["BANNERS"] as $index => $banner) : ?>
                <div class="carousel-item <?=$index == 0 ? 'active' : '' ?>">
                    <a target="_blank" href="<?= $banner['LINK'] ?>">
                        <img src="<?= $banner["DETAIL_SRC"] ?>" class="banner-desk <?if($banner["PREVIEW_SRC"]):?>banner-desk_hid<?endif;?> w-100" alt="">
                        <?if($banner["PREVIEW_SRC"]):?>
                            <img src="<?= $banner["PREVIEW_SRC"] ?>" class="banner-mob w-100" alt="">
                        <?endif;?>
                    </a>
                </div>
            <? endforeach; ?>
