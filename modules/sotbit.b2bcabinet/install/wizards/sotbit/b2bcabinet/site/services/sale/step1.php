...
if( $bRus || COption::GetOptionString( $moduleId, "wizard_installed", "N", WIZARD_SITE_ID ) != "Y" || WIZARD_INSTALL_DEMO_DATA ) {
    ...
    if($methodInstall == 'AS_TEMPLATE') {
        $path = WIZARD_SITE_DIR . 'b2bcabinet/';
    } else {
        $path = WIZARD_SITE_DIR;
    }

    Option::set($moduleId, 'ADDRESS_COMPANY', $path . "personal/buyer/profile_detail.php?ID=#ID#", WIZARD_SITE_ID);
    Option::set($moduleId, 'ADDRESS_ORDER', $path . "orders/detail/#ID#/", WIZARD_SITE_ID);
}
?>