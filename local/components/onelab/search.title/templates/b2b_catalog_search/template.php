...
if (isset($_GET['q']) && is_string($_GET['q']) && trim($_GET['q']) !== '') {
    $searchQuery = htmlspecialcharsbx($_GET['q']);
}


if($arParams["SHOW_INPUT"] !== "N"):?>
<div id="<?echo $CONTAINER_ID?>" class="search-form 3 <?=$themeClass;?>" xmlns="http://www.w3.org/1999/html">
    <form action="<?=$arParams["PAGE"]?>" id="b2b-catalog-search">
        <div class="form-control-feedback form-control-feedback-start">
            <input id="<?echo $INPUT_ID?>" type="search" class="form-control" name="q" value="<?=$searchQuery ?>" placeholder="<?=Loc::getMessage('CT_BST_SEARCH_BUTTON')?>" autocomplete="off"/>
