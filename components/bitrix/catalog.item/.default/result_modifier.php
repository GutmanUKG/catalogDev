...
if (
    ...
    foreach ($offersPricesUnprepared as $key => $price) {
    ...
    foreach ($item['OFFERS'] as $key => &$offer) {
        ...
        if (!empty($arParams['ITEMS_PRIVAT_PRICES'][$offer['ID']][$arParams['PRIVAT_PRICES_PARAMS']["PRICE_COLUMN"]])) {
            ...
            if (
                ...
                $item['MIN_PRICE']['PRIVATE_PRICE'] > $offersPrices[$offer['ID']]['PRIVATE_PRICE'][$offer['ITEM_QUANTITY_RANGE_SELECTED']]['PRICE']
                ) {
                $item['MIN_PRICE']['PRIVATE_PRICE'] = $offersPrices[$offer['ID']]['PRIVATE_PRICE'][$offer['ITEM_QUANTITY_RANGE_SELECTED']]['PRICE'];
            }
        }
...
unset($_markdownStoreCategoryMap);
    ...
    if ($arRes = $res->GetNext()){
        ...
        $arResult['ITEM']['AMOUNT_STORE_51'] = $str;
    }
