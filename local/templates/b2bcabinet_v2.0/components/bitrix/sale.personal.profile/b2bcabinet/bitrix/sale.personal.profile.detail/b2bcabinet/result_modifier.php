...
if(Loader::includeModule('sale') && Loader::includeModule('iblock'))
{
    ...
    $innProps = unserialize(Helper\Config::get('PROFILE_ORG_INN'), ['allowed_classes' => false]);
    //Заполение пустых полей , по факту они дублируют друг друга
    global $USER;
    $USER_ID = $arResult['USER_ID'];
    $FULL_NAME = $USER->GetFullName();
    if(empty($arResult['ORDER_PROPS_VALUES']['ORDER_PROP_1'])){
        $arResult['ORDER_PROPS_VALUES']['ORDER_PROP_1'] = $FULL_NAME;
    }
    if(empty($arResult['ORDER_PROPS_VALUES']['ORDER_PROP_6'])){
        $arResult['ORDER_PROPS_VALUES']['ORDER_PROP_6'] = $FULL_NAME;
    }
    if(empty($arResult['ORDER_PROPS_VALUES']['ORDER_PROP_11'])){
        $arLocs =  CSaleLocation::GetByID($arResult['ORDER_PROPS_VALUES']['ORDER_PROP_12'], LANGUAGE_ID);
        $arResult['ORDER_PROPS_VALUES']['ORDER_PROP_11'] = $arLocs['CITY_NAME'];
    }
