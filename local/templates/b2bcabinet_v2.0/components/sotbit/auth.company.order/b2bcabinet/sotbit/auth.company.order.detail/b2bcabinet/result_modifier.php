...
foreach ($arResult['ORDER_PROPS'] as $key => $ORDER_PROP) {
    if (!empty($ORDER_PROP['VALUE'])) {
        if (!($ORDER_PROP['MULTIPLE'] == "Y" && empty(array_diff(is_array(unserialize($ORDER_PROP['VALUE'], ['allowed_classes' => false])) ? unserialize($ORDER_PROP['VALUE'], ['allowed_classes' => false]) : [], [""]))))
